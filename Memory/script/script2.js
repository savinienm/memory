    ...
    for (let i = 0; i < tabJeu.length; i++) {
      ...
      for (let j = 0; j < tabJeu[i].length; j++) {
        if (tabJeu[i][j] === 0) {
          ...
          txt +=
            "<button id='"+ i + j +"' class='btn btn-warning taille' onClick='verif(\""+i +"-" +j +"\")'>Kaamelott</button>";       // On génère 4 boutons par ligne. La fonction onClick récupère les coordonnées i et j du bouton cliqué
        } else {
          txt += "<img id='"+ i + j +"' src='" + getImages(tabJeu[i][j]) + "' />";        //ici on récupère une image avec la fonction getImage()
        }
      }
      txt += "</div>";
    }
        ...
        setTimeout(() => {
            if(tabJeu[ligne][colonne] !== tabResultat[verifImage[0]][verifImage[1]]) {
                ...
                $(document).ready(function () {
                    ...
                    while (lig != tabJeu.length) {
                        col = 0;
                        while (col != tabJeu[lig].length) {
                            if (tabJeu[lig][col] == tabJeu[verifImage[0]][verifImage[1]]) {
                                nbOccur++;
                                tabID.push(lig);
                                tabID.push(col);
                                if(nbOccur == 2) {
